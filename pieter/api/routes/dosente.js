...
router.post('/assign/module',jsonParser,(req,res,next)=>{
  var sql = 'SELECT * FROM dosent WHERE NwuNumber = ?';
  var sql2 = 'SELECT * FROM module WHERE ModuleCode = ?';
  var sql3 ='INSERT INTO dosentmodule SET ?';
  var arr = [];
  var connection;
  con.getConnection()
  .then(conn=>{
    connection = conn;
   return conn.query(sql,req.body.nwunumber9);


  }).then(rows=>{
    
    arr.push(rows[0].DosentId);
   
    return connection.query(sql2,req.body.code);
    ...
    var post ={ModuleId:rows[0].ModuleId, DosentId:arr[0] };
    console.log(post);
    connection.query(sql3,post,(err,results,fields)=>{
      if(err)
      {
      ...
      {res.status(200).json({message:'Lecturer assigned'});}
